/**
 * Telegram Login Handler (English login page only)
 * Verifies Telegram payload on backend before creating local session.
 */

class TelegramLoginHandlerEn {
    constructor() {
    ...
    async verifyWithBackend(telegramPayload) {
        const response = await fetch(window.buildApiUrl('/api/auth/telegram/verify'), {
            method: 'POST',
            headers: {
...
document.addEventListener('DOMContentLoaded', () => {
    const loginHandler = new TelegramLoginHandlerEn();
    window.telegramLoginHandler = loginHandler;
    window.TelegramLoginHandler = TelegramLoginHandlerEn;
});
